...
public class Ex4 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// getParameter메서드는 해당 파라미터를 전달받지 못했다면 null을 반환
		// name 파라미터의 값을 반환
		String name = request.getParameter("name");
		// age 파라미터의 값을 반환
		String age = request.getParameter("age");
		
		// 문자열로된 숫자 age 변수 값을 정수로 바꿔서 나이에 맞는 태어난 연도를 계산해서 화면에 출력하세요.
		// (태어난 연도 = 올해 - 나이)
		
		int i_age = Integer.parseInt(age);
		//int i_age = new Integer(age);	// 오토언박싱
		int year = 2021 - i_age;
...
//		// getRemotePort() -> Client의 포트	// 클라이언트의 pc에서 몇번 문으로 나왔는지
